...
################################### DDL FOR MANUAL INPUTS
      ...
      -- ################################### GLOVO MONTHLY STATUS
            -- CREATE OR REPLACE EXTERNAL TABLE `logistics-data-storage-staging.long_term_pricing.glovo_monthly_mechanism_in_use`
            -- (
            -- month DATE,
            -- entity_id STRING,
            -- mechanism_code STRING,
            -- mechanism_in_use BOOL
            -- )
            -- OPTIONS (
            -- format="GOOGLE_SHEETS",
            -- uris=["https://docs.google.com/spreadsheets/d/1ulLRME_ktq81A1HdWWPozxAtJxKAsl5r8Xh3an8aAnQ/edit#gid=0"],
            -- sheet_range="glovo_pm_in_use!A:D",
            -- skip_leading_rows=1
            -- );
      -- ###################################
...
################################### EVALUATION PER COUNTRY
  CREATE OR REPLACE TABLE `logistics-data-storage-staging.temp_pricing.compscore_evaluation_per_country` AS
  ...
  ################################# DECISION TREES
        ...
        , wbs_tree AS (
              ...
              LEFT JOIN legality
                    ON market.entity_id = legality.entity_id
                    AND market.quarter = legality.quarter    
                    AND legality.mechanism_code = "wbs"  
        )
...
################################### GLOVO MECHANISM IN USE

      -- CREATE OR REPLACE TABLE `logistics-data-storage-staging.temp_pricing.glovo_weekly_status` AS
      -- WITH date_dims AS (
      --       SELECT DISTINCT
      --       DATE_TRUNC(first_day_of_week, QUARTER) as quarter
      --       , first_day_of_week as init_week
      --       FROM `fulfillment-dwh-production.curated_data_shared_coredata.dates` 
      --       WHERE year >= 2023
      -- )
      ...
      -- , gv.mechanism_in_use
      -- , gv.mechanism_code
      -- FROM `logistics-data-storage-staging.long_term_pricing.glovo_monthly_mechanism_in_use` gv
      -- LEFT JOIN date_dims d
      --       ON init_week BETWEEN month AND LAST_DAY(month);

###################################

################################### SCORE DETAILED PER COUNTRY

  CREATE OR REPLACE TABLE `logistics-data-storage-staging.temp_pricing.compliance_score_tracking` AS
      ...
      FROM `logistics-data-storage-staging.temp_pricing.compscore_evaluation_per_country`
      ...
      , add_glovo AS (
      SELECT *
      FROM unpivot_mechanism_in_use

      UNION ALL 

      SELECT *
      FROM `logistics-data-storage-staging.temp_pricing.glovo_weekly_status`
      )

      , join_values AS (
      SELECT a.* EXCEPT(should_have)
      , b.init_week
      , ed.management_entity
      -- , opm.n_orders
      , CASE 
            WHEN b.in_use = TRUE THEN 1 
            WHEN a.should_have = TRUE THEN 0
            ELSE NULL 
      END  as is_compliant
      , CASE 
            WHEN b.in_use = TRUE THEN 1 
            WHEN a.should_have = TRUE THEN 1
            ELSE 0
      END AS should_have

      FROM load_mechanism_to_comply a

      LEFT JOIN add_glovo b
...
################################### SCORE PER PLATFORM / GLOBAL
      ...
      CREATE OR REPLACE TABLE `logistics-data-storage-staging.temp_pricing.compliance_score_tracking_aggregated` AS
      ...
      FROM `logistics-data-storage-staging.temp_pricing.compliance_score_tracking`
      GROUP BY 1,2,3
      )

      , orders_per_market AS (
      select
            DATE_TRUNC(partition_date_local, ISOWEEK) as init_week
            , global_entity_id as entity_id
            , COUNT(order_id) as entity_orders
      from `fulfillment-dwh-production.curated_data_shared_coredata_business.orders`
      where partition_date_local >= DATE_SUB(filter_period, INTERVAL 12 WEEK)
      AND vertical_parent = "Food"
      AND is_delivery_by_dh
      AND is_net_order
      GROUP BY 1,2
      )
      ...
      WHERE lcs.init_week < DATE_TRUNC(CURRENT_DATE(), WEEK)

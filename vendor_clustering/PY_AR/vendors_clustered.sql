...
normalized_kpis as (
  select 
    *,
    percent_rank() over (partition by entity_id, city_name order by avg_basket asc) p_avg_basket,
    percent_rank() over (partition by entity_id, city_name order by avg_distance asc) p_avg_distance,
    ((avg_basket - avg(avg_basket) over()) / nullif(stddev_pop(avg_basket) over (), 0)) AS avg_basket_normalized,
    ((avg_distance - avg(avg_distance) over()) / nullif(stddev_pop(avg_distance) over (), 0)) AS distance_normalized,

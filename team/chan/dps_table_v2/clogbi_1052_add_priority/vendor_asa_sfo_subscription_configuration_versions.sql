...
--         DESCRIPTION: This table contains all historical information about Vendor ASA/SF Override/Subscriptions/Priority Fee Overrides price configuration, on version level. 
...
--               NOTES:
--             UPDATED: 2024-02-21 | Sebastian Lafaurie | CLOGBI-1052 | Add Priority Fee overrides
--                      ----------------------------------

CREATE OR REPLACE TABLE `logistics-data-storage-staging.temp_pricing.vendor_asa_sfo_subscription_configuration_versions`
...
WITH all_vendor_assignments AS (
  ...
  SELECT * FROM `fulfillment-dwh-production.cl._pricing_asa_vendor_assignments`
  UNION ALL SELECT * FROM `fulfillment-dwh-production.cl._pricing_sf_override_vendor_assignments`
  UNION ALL SELECT * FROM `fulfillment-dwh-production.cl._pricing_subscription_vendor_assignments`
  UNION ALL SELECT * FROM `logistics-data-storage-staging.temp_pricing._pricing_priority_fee_override_vendor_assignments`
...
-- ASA Price Configuration --Get all pricing ASA configurations.
  ...
  FROM `logistics-data-storage-staging.temp_pricing._pricing_asa_configuration_versions`

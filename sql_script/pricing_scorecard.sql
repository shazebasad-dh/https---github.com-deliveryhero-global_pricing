...
## SET PARTITION DATE
IF backfill
  THEN SET start_date_filter = DATE_SUB("2023-01-01", INTERVAL 0 DAY);
  ELSE SET start_date_filter = DATE_TRUNC(DATE_SUB(end_date_filter, INTERVAL 2 MONTH), MONTH);
END IF;
# STAGING TABLE
...
categories AS (
  ...
  FROM
    `logistics-data-storage-staging.long_term_pricing.market_archetypes`
  WHERE
    date >= DATE_SUB(start_date_filter, INTERVAL 2 DAY)
...
, population AS (
  ...
  WHERE month BETWEEN start_date_filter AND end_date_filter
)
, total_orders AS (
  SELECT
  ...
  WHERE o.partition_date_local >= DATE_SUB(start_date_filter, INTERVAL 2 DAY) AND end_date_filter < DATE_ADD(end_date_filter, INTERVAL 2 DAY)
    AND o.is_successful
...
, kpi AS (
  SELECT
    ...
    COUNT(DISTINCT CASE WHEN (o.vertical_type IN ('restaurants', 'coffee', 'street_food')) AND is_own_delivery THEN platform_order_code END) AS od_restaurant_orders,
    COUNT(DISTINCT CASE WHEN (o.vertical_type IN ('restaurants', 'coffee', 'street_food')) AND is_own_delivery THEN platform_order_code END)/t.orders AS od_rest_orders_share,
    COUNT(DISTINCT CASE WHEN (o.vertical_type IN ('restaurants', 'coffee', 'street_food')) AND is_own_delivery THEN analytical_customer_id END) od_restaurant_users,
    ...
    -- SUM(discount_dh_eur + voucher_dh_eur)/COUNT(DISTINCT platform_order_code) incentives_dh_eur,
    ...
    -- COUNT(DISTINCT CASE WHEN has_new_customer_condition THEN platform_order_code END) AS new_customer_orders,
    -- COUNT(DISTINCT CASE WHEN has_new_customer_condition THEN platform_order_code END)/COUNT(DISTINCT platform_order_code) AS new_customer_orders_share,
    CASE
      WHEN o.entity_id IN ("PY_CL", "PY_DO", "PY_EC", "PY_SV", "PY_CR", "PY_GT") THEN AVG(weighted_df_diff_ubereats)
      WHEN o.entity_id NOT IN ("PY_CL", "PY_DO", "PY_EC", "PY_SV", "PY_CR", "PY_GT") THEN AVG(weighted_df_diff_rappi)
      WHEN o.region = "Europe" THEN AVG(weighted_df_diff_vs_comp)
      WHEN o.region = "Asia" THEN AVG(weighted_df_diff_grab)
    END AS weighted_df_diff_vs_comp,
  ...
  LEFT JOIN categories c ON o.entity_id = c.entity_id AND DATE_TRUNC(created_date_local, month) = c.month
  ...
  LEFT JOIN `logistics-data-storage-staging.long_term_pricing.point_of_marginal_expensiveness` pme ON o.country_code = pme.country_code AND DATE_TRUNC(o.created_date_local, month) = pme.month
...
, tests AS (
  ...
  WHERE DATE(t.test_start_date) >= DATE_SUB(start_date_filter, INTERVAL 2 DAY)
...
, avg_kpi AS (
  ...
  WINDOW
    agg_region AS (PARTITION BY kpi.region, kpi.month),
...
, kpi_score AS (
  SELECT
    ...
    CASE WHEN test_all_entities_count_cum >= total_tests_reg_threshold THEN 1 WHEN test_all_entities_count_cum IS NOT NULL THEN 0 END AS total_tests_score,
    ...
    CASE WHEN test_all_entities_count_cum >= total_tests_ma_threshold THEN 1 WHEN test_all_entities_count_cum IS NOT NULL THEN 0 END AS total_tests_ma_score,
...
SELECT
  kpi.*,
  pm.* EXCEPT(region, entity_id, month),
  tests.* EXCEPT(entity_id, month),
  SAFE_DIVIDE((profitable_orders_score + free_df_scheme_score + campaign_orders_score + mov_afv_score +surge_df_score +sbf_cf_score + sf_cf_score),
  ...
  SAFE_DIVIDE((diff_df_comp_score + churn_responses_score + share_below_pme_score),
    IF(diff_df_comp_score IS NOT NULL,1,0) + IF(churn_responses_score IS NOT NULL,1,0) + IF(share_below_pme_score IS NOT NULL,1,0)) AS affordability_score,
  SAFE_DIVIDE((od_rest_orders_score+frequency_score+user_penetration_score),
  ...
  SAFE_DIVIDE((valid_tests_score + total_tests_score),
    (IF(valid_tests_score IS NOT NULL,1,0) + IF(total_tests_score IS NOT NULL,1,0))) AS experimentation_score,
  SAFE_DIVIDE((profitable_orders_ma_score + free_df_scheme_ma_score + campaign_orders_ma_score+ mov_afv_ma_score+ surge_df_ma_score+ sbf_cf_ma_score+ sf_cf_ma_score),
  ...
  SAFE_DIVIDE((diff_df_comp_ma_score + churn_responses_ma_score + share_below_pme_ma_score),
    (IF(diff_df_comp_ma_score IS NOT NULL,1,0) + IF(churn_responses_ma_score IS NOT NULL,1,0) + IF(share_below_pme_ma_score IS NOT NULL,1,0))) AS affordability_ma_score,
  SAFE_DIVIDE((od_rest_orders_ma_score + frequency_ma_score + user_penetration_ma_score),
  ...
  SAFE_DIVIDE((valid_tests_ma_score + total_tests_ma_score),
...
IF backfill THEN
  CREATE OR REPLACE TABLE `logistics-data-storage-staging.long_term_pricing.pricing_scorecard`
    PARTITION by month OPTIONS (partition_expiration_days = null, require_partition_filter = false) AS
  SELECT * FROM staging_table;
ELSE
  MERGE INTO `logistics-data-storage-staging.long_term_pricing.pricing_scorecard` s
  USING staging_table stg ON prd.entity_id = stg.entity_id AND prd.month = stg.month
WHEN MATCHED THEN
  UPDATE SET
    region = stg.region

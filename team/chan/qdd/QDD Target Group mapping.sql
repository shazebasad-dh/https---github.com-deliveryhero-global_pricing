...
SET test_arrays = [
"KW_20221111_G_C_P_GroceriesMOV"
];
...
WITH average_commission AS(
    ...
    , real_df AS(
        ...
        CASE
          WHEN  target_group IS NULL THEN NULL 
          WHEN target_group IN ("Target Group 1", "Target Group 2") then "Target Group 1-2"
          ELSE "Target Group 3-12"
        END as target_group
...
########### SESSION DATA
    ...
    , users AS (
        ...
        , CASE
          WHEN  target_group IS NULL THEN NULL 
          WHEN target_group = "All" then target_group
          WHEN target_group IN ("Target Group 1", "Target Group 2") then "Target Group 1-2"
          ELSE "Target Group 3-12"
        END as target_group
...
######## AVG METRICS
    ...
    SELECT * FROM add_test_list_info ORDER BY code;

############ PERSIST TABLES
    -- Persist Data
    EXECUTE IMMEDIATE ('''
        CREATE OR REPLACE TABLE `dh-logistics-product-ops.pricing._'''|| upper(CtryCode) ||'''_AB_Results_KW_20221111_G_C_P_GroceriesMOV`
        AS
        SELECT
        *
        FROM staging_orders;
